...
@section('ContentHeader')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">{{trans('admin/role.create')}}</li>
    </ol>
@endsection
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">{{trans('admin/role.create')}}</h3>
        </div>
        <!-- /.box-header -->
        <!-- form start -->
        <form method="post" action="{{url('admin/role')}}"  id="createForm">
            <div class="box-body">
                {!!csrf_field()!!}
                <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                    <label for="cn_name">{{trans('admin/role.model.name')}}</label>
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-control" placeholder="{{trans('admin/role.model.name')}}">
                </div>
                <div class="form-group{{ $errors->has('display_name') ? ' has-error' : '' }}">
                    <label for="cn_name">{{trans('admin/role.model.display_name')}}</label>
                    <input type="text" name="display_name" value="{{ old('display_name') }}" id="display_name" class="form-control" placeholder="{{trans('admin/role.model.display_name')}}">
                </div>

                <div class="form-group">
                    <label for="sort">{{trans('admin/role.model.description')}}</label>
                    <input type="text" name="description" value="{{ old('description') }}" id="description"  class="form-control" placeholder="{{trans('admin/role.model.description')}}">
                </div>
                <div class="form-group">
                    ...
                    <div class="checkbox">
                        ...
                        <label>
                            <input type="checkbox" name="permissions[]" value="{{$childValue['permission_id']}}" />
