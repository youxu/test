...
@section('content')
    <div class="box box-primary">
        ...
        <!-- /.box-header -->
        <!-- form start -->
        <form method="post" action="{{url('admin/contro',[$contro->id])}}"  id="createForm">
            <div class="box-body">
                {!!csrf_field()!!}
                {{method_field('PUT')}}
                ...
                <div class="form-group{{ $errors->has('func_name') ? ' has-error' : '' }}">
                    ...
                    <input type="text" name="func_name" value="{{ $contro->func_name }}" id="func_name" class="form-control" placeholder="{{trans('admin/contro.model.func_name')}}">
                </div>
                <div class="form-group{{ $errors->has('func_name_cn') ? ' has-error' : '' }}">
                    <label for="func_name_cn">{{trans('admin/contro.model.func_name_cn')}}</label>
                    <input type="text" name="func_name_cn" value="{{ $contro->func_name_cn }}" id="func_name_cn" class="form-control" placeholder="{{trans('admin/contro.model.func_name_cn')}}">
                </div>
                <div class="form-group">
                    <label for="sort">{{trans('admin/contro.model.icon')}}</label>
                    <input type="text" name="icon" value="{{ $contro->icon }}" id="icon"  class="form-control" placeholder="{{trans('admin/contro.model.icon')}}">
                </div>
                <div class="form-group">
                    <label for="sort">{{trans('admin/contro.model.order_num')}}</label>
                    <input type="text" name="order_num" value="{{ $contro->order_num }}" id="order_num"  class="form-control" placeholder="{{trans('admin/contro.model.order_num')}}">
                </div>
                <div class="form-group">
                    <label for="is_menu">{{trans('admin/contro.model.is_menu')}}</label>
                    <div class="form-inline">
                        <input type="radio" name="is_menu"  {{is_checked(1,$contro->is_menu)}} value="1">{{trans('admin/action.radio_true')}}
                        <input type="radio" name="is_menu" {{is_checked(0,$contro->is_menu)}} value="0">{{trans('admin/action.radio_false')}}
                    </div>
                </div>
            </div>
            <!-- /.box-body -->
